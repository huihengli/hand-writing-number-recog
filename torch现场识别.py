...
class SimpleCNN(nn.Module):
    def __init__(self):
        ...
        # 定义第一个卷积层，输入通道数为1，输出通道数为32，卷积核大小为3x3，padding为1，所谓通道相比于彩色照片的rgb，这里是灰度图，只有一个通道，彩色有3个
        self.conv1 = nn.Conv2d(1, 32, kernel_size=3, padding=1)
        # 定义第二个卷积层，输入通道数为32，输出通道数为64，卷积核大小为3x3，padding为1
        ...
        # 定义第三个全连接层，输入大小为64，输出大小为10（对应10个类别）
...
class DigitRecognizerApp:
    def __init__(self, model):
        ...
        self.predict_button = tk.Button(self.button_frame, text='识别', command=self.predict_digit)
        ...
        self.label = tk.Label(self.window, text='', font=('Arial', 18))
        self.label.pack()
    ...
    def predict_digit(self):
        ...
        with torch.no_grad():
            ...
            self.label.config(text=f"预测结果：{predicted}")
